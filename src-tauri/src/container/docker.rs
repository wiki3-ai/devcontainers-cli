//! Docker backend — placeholder. The runtime reports `Unavailable` until
...
impl ContainerRuntime for DockerRuntime {
    ...
    async fn probe(&self) -> Result<RuntimeAvailability, ContainerRuntimeError> {
        Ok(RuntimeAvailability {
            available: false,
            version: None,
            reason: Some("Docker backend lands in Phase 3 of the conversion roadmap".into()),
        })
    }
...
fn unsupported() -> ContainerRuntimeError {
    ContainerRuntimeError::Unsupported("Docker backend not implemented".into())
}
