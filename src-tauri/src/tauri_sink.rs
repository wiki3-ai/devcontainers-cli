//! [`TauriSink`] — the Devcontainers.app binary's [`EventSink`] impl.
//!
//! Forwards orchestrator events onto the Tauri event bus as
//! `devcontainer://status` and `devcontainer://log` events. Sibling
//! apps that embed [`devcontainer_core`] provide their own sink with
//! whatever event names their frontend expects.
