...
// `Mount` is inlined here because the legacy
// `spec-configuration/containerFeaturesConfiguration.ts` drags in `tar`/`OCI`
// and is intentionally not part of this slice.
...
export async function getDockerComposeFilePaths(cliHost: FileHost, config: DevContainerFromDockerComposeConfig, envForComposeFile: NodeJS.ProcessEnv, cwdForDefaultFiles: string) {
	...
	try {
		...
		const match = /^COMPOSE_FILE=(.+)$/m.exec(new TextDecoder().decode(buffer));
